({
      ...
      switch (type) {
        case 'created':
          ...
          activities.sort(function(a, b) {
            return (a.days_ago__c > b.days_ago__c) ? 1 : ((b.days_ago__c >
              a.days_ago__c) ? -1 : 0);
          });
  ...
  getActivitiesForTrip: function(component) {
    var action = component.get('c.getActivitiesForTrip');
    var id = component.get('v.recordId');
  ...
  getActivitiesForObject: function(component) {
    var action = component.get('c.getActivitiesForObject');
    var id = component.get('v.recordId');
    var relationship = component.get('v.relationshipName');
    action.setParams({
      recordId: id,
  ...
  getContactForAccount: function(component) {
    ...
    action.setCallback(this, function(response) {
      ...
      if (component.isValid() && state === "SUCCESS") {
        ...
        console.log('getContactForAccount: ' + response.getReturnValue());
      } else {
        console.log("getContactForAccount Failed with state" + state);
      }
    })
    $A.enqueueAction(action);

  },
  getContactForChat: function(component) {
    var action = component.get('c.getContactForChat');
    var id = component.get('v.recordId');
    ...
    action.setCallback(this, function(response) {
      ...
      if (component.isValid() && state === "SUCCESS") {
        component.set("v.contactId", response.getReturnValue());
        console.log('---- getContactForChat return value: ' + response.getReturnValue());
      } else {
        console.log("---- getContactForChat Failed with state" + state);
      }
    })
    $A.enqueueAction(action);

  },
  getActivitiesByContactId: function(component) {
    var action = component.get('c.getActivitiesForContact');
    var id = component.get('v.contactId');
    console.log('****** getActivitiesByContactId:' + id);
    ...
    action.setCallback(this, function(response) {
      ...
      if (component.isValid() && state === "SUCCESS") {
        ...
        console.log("getActivitiesByContactId - Failed with state" + state);
      }
    })
    $A.enqueueAction(action);
  },
  getActivitiesForLiveChat: function(component) {

    console.log('***** inside getActivitiesForLiveChat...');

    var action = component.get('c.getActivitiesForLiveChat');
    var id = component.get('v.recordId');
    ...
    action.setCallback(this, function(response) {
      ...
      if (component.isValid() && state === "SUCCESS") {
        ...
        component.set("v.activities", response.getReturnValue());
        console.log('getActivitiesForLiveChat: ' + response.getReturnValue());


      } else {
        console.log("getActivitiesForLiveChat Failed with state" + state);
      }
    })
    $A.enqueueAction(action);
  }
